...
**Working name:** `ShieldLab` (folder `shieldlab/`)
...
├── shieldlab/                 ← editable Python package
...
| 2026-06-10 | 2 — Engine | **COMPLETE & validated.** Built `shieldlab/` package: `data_loader.py` (cached JSON access + citation resolver), `physics/transmission.py` (Archer, TVL, mu+buildup models + analytic inverses + multi-layer combiner), `physics/beams.py` (maps modality/energy/material -> correct data+model), `physics/barriers.py` (layered Barrier, areal density, equivalent-thickness), `physics/sources.py` (diagnostic/CT/LINAC/radionuclide source terms + I-131 RG-8.39 biokinetics), `regulatory/limits.py` (NCRP & IAEA/NRRC design goals + pass/fail verdict), `physics/solver.py` (evaluate barrier, required & preferred thickness). Smoke test passes: SRS 47 Co-60 primary barrier = **1034 mm concrete (expected 1033)**; RG 8.39 = **4.53 mSv (exact)**; TVL/Archer sanity OK. |
...
| 2026-06-11 | 4 — Validation & release | **COMPLETE.** `shieldlab/report/report.py` (self-contained HTML audit report → print to PDF; wired into UI download button). `tests/test_validation.py` (13 tests: transmission models, analytic-inverse round-trips, SRS 47 Co-60, RG 8.39 examples ×2, PET 511 keV TVL, multilayer product, verdict logic, report build, data-file load — all PASS). `README.md` (install, usage, method+references table, editing guide, structure, limitations). `requirements.txt`. Final integration verified. |
