...
$engine = Replace-Once $engine '    "still rising; other materials and energies are unquantified. An independent "' (@(
...
$test = Replace-Once $test '    assert "lead" in document' (@(
    '    assert "lead" in document',
    '    assert "precision gate" in document',
    '    assert "below 10%" in document'
) -join (if ($test.Contains("`r`n")) { "`r`n" } else { "`n" })) 'report assertions'
