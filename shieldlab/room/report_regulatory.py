...
def build_submission_html(report: Dict, meta: Dict,
    ...
    if biased_rows:
        ...
        geometry_bias_html = (
            ...
            f"{_esc(eng.GEOMETRY_BIAS_WARNING)} The factors come from Monte-Carlo runs that "
            f"widened the beam from 0.5&nbsp;m to 2.5&ndash;3.5&nbsp;m at fixed barrier and "
            f"detector. Concrete at 364&nbsp;keV converged by 2.5&nbsp;m; lead at 511&nbsp;keV "
            f"converged by 1.5&nbsp;m. Steel and concrete at 511&nbsp;keV still rose 5% and 8% "
            f"between 1.5 and 2.5&nbsp;m, so their true factors may be larger and these results "
            f"may still read <b>low</b>, the non-conservative direction. The out-of-domain guard "
            f"does not detect the condition (a barrier can have ordinary features), and the "
            f"analytical tier is not reliably the more conservative option here."
            f"</div>")
    ...
    finite_field_scope_html = (
        ...
        "broad beam. Every result is raised by a broad-beam factor measured for lead, steel and "
        "concrete at 364 and 511&nbsp;keV; other materials take the concrete factor, the largest "
        "measured, and other energies take the same factors untested. The deficit was already "
        "1.70&times; in concrete at &mu;x&nbsp;4, the shallowest depth measured, and below that "
        "the &mu;x&nbsp;4 factor is applied. The &mu;x&ge;4 red flag is a priority "
